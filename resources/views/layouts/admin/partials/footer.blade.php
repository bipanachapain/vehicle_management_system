...
<script>
 const labels = JSON.parse(document.getElementById("chart-labels").value);
    const data   = JSON.parse(document.getElementById("chart-data").value);
...
new Chart(ctx1, {
    ...
    options: {
        ...
        scales: {
            y: { beginAtZero: true },
            x: { ticks: { autoSkip: false } } // show all months
        }
    }
});
</script>
  ...
  {{-- <script>
    var timeline = @json($renewalsTimeline);
    ...
    new Chart(ctx1, {
        type: "line",
        data: {
            labels: timeline.labels,   // ["Aug", "Sep", ...]
            datasets: [{
                label: "Renewals",
                tension: 0.4,
                borderWidth: 3,
        ...
        options: {
            ...
            interaction: { intersect: false, mode: 'index' },
            scales: {
                y: { grid: { borderDash: [5, 5] }, ticks: { color: '#666' } },
...
{{-- <script>
   const timeline = @json($renewalsTimeline ?? ['labels' => [], 'counts' => []]);
    ...
    new Chart(ctx1, {
      ...
      options: {
        ...
        interaction: { intersect: false, mode: 'index' },
        scales: {
          y: { grid: { drawBorder: false, borderDash: [5,5] }, ticks: { padding: 10 } },
